...
MOSS 的音频声纹处理非常占显存, 单卡 22GB 只能承载十分钟出头的音频片段; 且受 MOSS 机制限制, 双卡显存不能线性叠加, 也不适合张量并行。因此对长音频分段处理: 单段约 810 秒, 段间各留 30 秒冗余用于特征对齐, 再交给双卡分段并行转写, 实测比单卡快约 50%。

MOSS 自身具备基于文本逻辑的角色识别, 但分段后各段独立推理, 角色编号可能对不齐。为此引入基于 CPU 的声纹聚类后处理, 通过可信度加权综合判断角色归属, 实现全局角色识别的高准确率。

实测对四川话识别率较高。换用更新的硬件会更快, 但显存仍是刚需。
