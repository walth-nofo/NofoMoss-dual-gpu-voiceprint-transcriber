...
> Nofo Diarized Local Recorder Transcriber (Dual-GPU)

基于 OpenMOSS 开源的 MOSS-Transcribe-Diarize 模型（0.9B）的自研服务层 —— 长音频分段、双卡并行推理、声纹聚类跨段统一说话人、Web UI。

用两张消费级显卡跑 0.9B 语音模型，把本地录音转写成带统一说话人标签的文本，全程本地、不上传。
...
MOSS 的音频声纹处理非常占显存, 2080ti单卡 22GB 只能承载十分钟出头的音频片段; 且受 MOSS 机制限制, 双卡显存不能线性叠加, 也不适合张量并行。因此对长音频分段处理: 单段约 810 秒, 段间各留 30 秒冗余用于特征对齐, 再交给双卡分段并行转写, 实测比单卡快约 50%。

MOSS 自身具备基于文本逻辑的角色识别, 但分段后各段独立推理, 角色编号可能对不齐。为此引入基于 CPU 的声纹聚类后处理, 通过可信度加权综合判断角色归属, 实现全局角色识别的高准确率。

实测对四川话识别率较高，即便语音时间线完全重合，也能够根据角色分别转译文本。其他方言效果尚不明确。换用更新的硬件会更快, 但显存仍是刚需。
...
![Nofo 双卡声纹本地录音转写工具 Web UI](docs/screenshots/web-ui.jpg)
...
```
router.py (:8000, 可选) = vLLM 模型路由/热切换,, 主要是为解决同本地大语言模型模型服务切换问题而设，与本系统耦合较松(不包含在本仓库)
